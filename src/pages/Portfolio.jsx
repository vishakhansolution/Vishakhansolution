          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 md:gap-x-12 gap-y-16 md:gap-y-24">
            {filteredProjects.map((project, idx) => (
              <div 
                ...
                data-aos="fade-up"
                data-aos-delay={idx % 3 * 100} // Staggered entry for the row
              >
                {/* Image Container with Fixed Aspect Ratio */}
                <div className="relative overflow-hidden aspect-[3/4] rounded-t-full bg-slate-50 transition-all duration-1000 group-hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.12)]">
                  <img 
                    src={project.img} 
                    alt={project.title} 
                    className="w-full h-full object-cover grayscale-[40%] group-hover:grayscale-0 group-hover:scale-110 transition-all duration-1000 ease-in-out"
                  />
                  
                  <div className="absolute inset-0 bg-slate-900/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
                  
                  <div className="absolute top-12 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur-sm px-5 py-1.5 text-[8px] font-bold tracking-[0.4em] uppercase text-slate-900 opacity-0 group-hover:opacity-100 transition-all duration-500 shadow-sm">
                    {project.category}
                  </div>
                </div>
                ...
                <div className="mt-8 md:mt-10 px-2">
                   <div className="flex items-center justify-between mb-4">
                      <span className="text-[9px] font-bold tracking-widest text-blue-600 uppercase">/ 0{idx + 1}</span>
                      <div className="flex items-center gap-2 text-[9px] text-slate-400 uppercase tracking-widest font-medium">
                         <Globe size={10} className="text-blue-600" /> {project.location}
                      </div>
                   </div>
                   <h3 className="text-xl md:text-2xl font-serif text-slate-900 mb-3 group-hover:italic transition-all duration-500">
                      {project.title}
                   </h3>
                   <div className="h-[1px] w-full bg-slate-100 group-hover:bg-blue-600 transition-colors duration-700"></div>
                </div>
              </div>
            ))}
          </div>
